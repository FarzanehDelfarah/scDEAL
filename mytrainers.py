...
def train_predictor_model(net, data_loaders, optimizer, loss_function, n_epochs, scheduler, load=False, save_path="model.pkl"):
    if load and os.path.exists(save_path):
        try:
            ...
            logging.warning("Failed to load existing file, proceeding with training.")
    ...
    for epoch in range(n_epochs):
        logging.info(f'Epoch {epoch}/{n_epochs - 1}')
        logging.info('-' * 10)
        ...
        for phase in ['train', 'val']:
            ...
            for batchidx, (x, y) in enumerate(data_loaders[phase]):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    if phase == 'train':
                        loss.backward(retain_graph=True)  # Retain the graph to help debug
                        optimizer.step()
            ...
            last_lr = scheduler.optimizer.param_groups[0]['lr']
            loss_train[epoch, phase] = epoch_loss
            logging.info(f'{phase} Loss: {epoch_loss:.8f}. Learning rate = {last_lr}')
