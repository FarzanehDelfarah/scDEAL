...
def run_main(args):
    #args.checkpoint = "save/models/ALL_gene_expression.csv_data_GSE165318_dose_DOSE2_bottle_512_edim_512,256_pdim_16,8_model_AE_dropout_0.3_gene_F_lr_0.01_mod_new_sam_no"
    ...
    # Replicate the expression data 9 times to match the label data
    data = np.tile(data, (9, 1))
    label = label_r.values.flatten()

    print("Data shape:", data.shape)
    print("Label shape:", label.shape)

        # Check if data and label are aligned correctly
    if data.shape[0] != label.shape[0]:
        raise ValueError("Mismatch in number of samples between data and label")

    # Extract labels
    label = label_r.loc[selected_idx, :]
    ...
    for column in label_r.columns:
        unique_values = label_r[column].unique()
        print(f"Unique values in column '{column}': {unique_values}")
 
    # Dictionary to store unique values
    unique_values_dict = {}

    # Populate the dictionary with unique values for each column
    for column in label_r.columns:
        unique_values_dict[column] = label_r[column].unique()
    ...
    if str(args.pretrain) != "False":
       ...
       if reduce_model == "VAE":
           encoder = VAEBase(input_dim=61958, latent_dim=dim_au_out, h_dims=encoder_hdims, drop_out=args.dropout)
       elif reduce_model in ['AE', 'DAE']:
           encoder = AEBase(input_dim=61958, latent_dim=dim_au_out, h_dims=encoder_hdims, drop_out=args.dropout)
...
# Define the model of predictor
    if reduce_model in ["AE", "DAE"]:
       model = PretrainedPredictor(
           input_dim=61958,  # Ensure this matches your input data dimensions
    ...
    model.to(device)
    bulk_model = "save/models/ALL_gene_expression.csv_data_GSE165318_dose_DOSE2_bottle_512_edim_512,256_pdim_16,8_model_AE_dropout_0.3_gene_F_lr_0.01_mod_new_sam_no"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--selected_dose", type=str, default="DOSE2", help="Selected dose for training")
