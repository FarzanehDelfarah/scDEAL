...
class AEBase(nn.Module):
    def __init__(self, input_dim, latent_dim, h_dims, drop_out=0.2):
        super(AEBase, self).__init__()
        self.encoder = nn.Sequential(
            nn.Linear(input_dim, h_dims[0]),  # First layer dimension
            nn.ReLU(),
            nn.Dropout(drop_out),
            nn.Linear(h_dims[0], latent_dim)  # Last layer dimension matches latent_dim
        )
        # Additional layers for decoder
        self.decoder_input = nn.Linear(latent_dim, h_dims[0])
        self.decoder = nn.Sequential(
            nn.ReLU(),
            nn.Dropout(drop_out),
            nn.Linear(h_dims[0], input_dim)  # Output dimension matches the input dimension
        )
...
class PretrainedPredictor(nn.Module):
    def __init__(self, input_dim, latent_dim, h_dims, hidden_dims_predictor, output_dim, pretrained_weights=None, freezed=True, drop_out=0.2, drop_out_predictor=0.2):
        super(PretrainedPredictor, self).__init__()
        self.latent_dim = latent_dim  # Ensure latent_dim is set
        self.encoder = AEBase(input_dim, latent_dim, h_dims, drop_out)
        ...
        self.predictor = nn.Sequential(
            nn.Linear(latent_dim, hidden_dims_predictor[0]),  # First layer of predictor
            nn.ReLU(),
            nn.Dropout(drop_out_predictor),
            nn.Linear(hidden_dims_predictor[0], output_dim)  # Output dimension of predictor
        )
...
class PretrainedAEPredictor(AEBase):
    def __init__(self,
                 input_dim,
                 latent_dim=128,
                 h_dims=[512],
