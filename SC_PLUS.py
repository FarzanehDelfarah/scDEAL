...
def run_main(args):
    ...
    if reduce_model in ["AE", "DAE"]:
        Xtarget_trainTensor_encoded = encoder.encode(Xtarget_trainTensor)
        Xtarget_validTensor_encoded = encoder.encode(Xtarget_validTensor)
        X_allTensor_encoded = encoder.encode(X_allTensor)
    elif reduce_model == "VAE":
        Xtarget_trainTensor_encoded = encoder.encode(Xtarget_trainTensor, repram=True)
        Xtarget_validTensor_encoded = encoder.encode(Xtarget_validTensor, repram=True)
        X_allTensor_encoded = encoder.encode(X_allTensor, repram=True)
    else:
        raise ValueError(f"Invalid reduce_model value: {reduce_model}")

    # Log tensor shapes for debugging
    logging.info(f"Xtarget_trainTensor_encoded shape: {Xtarget_trainTensor_encoded.shape}")
    logging.info(f"Xtarget_validTensor_encoded shape: {Xtarget_validTensor_encoded.shape}")
    logging.info(f"X_allTensor_encoded shape: {X_allTensor_encoded.shape}")



    # Create DataLoaders for the encoded data
    train_dataset_encoded = TensorDataset(Xtarget_trainTensor_encoded, Ctarget_trainTensor)
    valid_dataset_encoded = TensorDataset(Xtarget_validTensor_encoded, Ctarget_validTensor)


    Xtarget_trainDataLoader_encoded = DataLoader(dataset=train_dataset_encoded, batch_size=batch_size, shuffle=True)
    Xtarget_validDataLoader_encoded = DataLoader(dataset=valid_dataset_encoded, batch_size=batch_size, shuffle=True)
    
    dataloaders_pretrain = {'train': Xtarget_trainDataLoader_encoded, 'val': Xtarget_validDataLoader_encoded}
    ...
    # Encode the source training and validation data before creating DataLoaders
    if reduce_model in ["AE", "DAE"]:
        Xsource_trainTensor_encoded = encoder.encode(Xsource_trainTensor)
        Xsource_validTensor_encoded = encoder.encode(Xsource_validTensor)
    elif reduce_model == "VAE":
        Xsource_trainTensor_encoded = encoder.encode(Xsource_trainTensor, repram=True)
        Xsource_validTensor_encoded = encoder.encode(Xsource_validTensor, repram=True)
    else:
        raise ValueError(f"Invalid reduce_model value: {reduce_model}")

    # Log tensor shapes for debugging
    logging.info(f"Xsource_trainTensor_encoded shape: {Xsource_trainTensor_encoded.shape}")
    logging.info(f"Xsource_validTensor_encoded shape: {Xsource_validTensor_encoded.shape}")


    # Create TensorDatasets using the encoded tensors
    sourcetrain_dataset_encoded = TensorDataset(Xsource_trainTensor_encoded, Ysource_trainTensor)
    sourcevalid_dataset_encoded = TensorDataset(Xsource_validTensor_encoded, Ysource_validTensor)

    # Create DataLoaders for the encoded data
    Xsource_trainDataLoader_encoded = DataLoader(dataset=sourcetrain_dataset_encoded, batch_size=batch_size, shuffle=True)
    Xsource_validDataLoader_encoded = DataLoader(dataset=sourcevalid_dataset_encoded, batch_size=batch_size, shuffle=True)

    # Assign DataLoaders to the dataloaders_source dictionary
    dataloaders_source = {'train': Xsource_trainDataLoader_encoded, 'val': Xsource_validDataLoader_encoded}    ################################################# END SECTION OF LOADING BULK DATA #################################################
